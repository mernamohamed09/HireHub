...
module.exports = {
  init: (httpServer) => {
    ...
    // Authenticate every connection from its JWT and derive the room from the
    // verified token. The old code let any client join any user's room just by
    // emitting their id, exposing that user's notifications, chat and AI events.
    io.use((socket, next) => {
