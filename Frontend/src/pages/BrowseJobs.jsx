  ...
  return (
    <>
      <div className="max-w-7xl mx-auto px-4 pb-12 w-full pt-8">
        {/* Header Section */}
        <div className="glass-card-pro border-white/5 p-6 md:p-8 rounded-2xl mb-8 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6 relative z-10">
            <div>
              <h1 className="font-bold text-2xl md:text-3xl text-white mb-2">Find Your Dream Role</h1>
              <p className="text-on-surface-variant text-xs md:text-sm">Showing {jobs.length} job{jobs.length !== 1 ? 's' : ''}{total > jobs.length ? ` of ${total}` : ''} available now.</p>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 w-full md:w-auto">
              <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider">Sort by:</span>
              <select
                className="bg-surface-container border border-white/10 rounded-xl text-white text-sm px-4 py-2 w-full sm:w-auto focus:border-emerald-400 focus:shadow-[0_0_8px_rgba(16,185,129,0.15)] outline-none transition-all"
                value={sort}
