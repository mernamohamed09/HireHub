...
export function Profile() {
  ...
  return (
    <div className="w-full">
      {/* Header Profile Card */}
      <header className="glass-panel rounded-2xl p-8 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden group">
        <div className="absolute -top-10 -right-10 w-48 h-48 bg-neon-purple/20 blur-3xl rounded-full"></div>
        <div className="flex items-center gap-6 relative z-10">
          <div className="relative group/avatar">
            ...
            <button
              ...
              className="absolute -bottom-2 -right-2 w-10 h-10 rounded-xl bg-neon-purple text-white flex items-center justify-center shadow-glow-purple hover:scale-105 transition-all disabled:opacity-60"
            >
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-on-surface-variant font-medium">
              <span className="bg-surface-container-high px-3 py-1 rounded-md border border-white/5 text-neon-cyan">{profile?.title || 'No title added yet'}</span>
              {user?.location && (
                <span className="flex items-center gap-1 bg-surface-container-high px-3 py-1 rounded-md border border-white/5"><MapPin size={14} className="text-neon-orange" />{user.location}</span>
              )}
            </div>
          </div>
        </div>
      ...
      {isEditing && (
        <form onSubmit={handleSave} className="glass-panel p-8 mb-8 space-y-6 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap relative z-10">
            ...
            <Button
              ...
              className="flex items-center gap-2 border-neon-cyan/50 text-neon-cyan hover:bg-neon-cyan hover:text-black transition-all"
          ...
          <div className="relative z-10">
            ...
            <input
              className="w-full bg-surface-container border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-neon-purple focus:shadow-glow-purple transition-all"
              value={form.title}
          ...
          <div className="relative z-10">
            ...
            <textarea
              className="w-full bg-surface-container border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-neon-purple focus:shadow-glow-purple transition-all resize-none"
              rows={3}
              value={form.bio}
          ...
          <fieldset className="border border-white/10 rounded-2xl p-6 relative z-10 bg-surface-container/50">
            <legend className="px-2 text-[10px] font-bold text-neon-cyan uppercase tracking-wider">Professional links</legend>
          ...
          <fieldset className="border border-white/10 rounded-2xl p-6 space-y-6 relative z-10 bg-surface-container/50">
            <div className="flex items-center justify-between">
              <legend className="text-[10px] font-bold text-neon-cyan uppercase tracking-wider">Work experience</legend>
              <Button
                type="button"
                variant="ghost"
                className="text-neon-cyan border border-neon-cyan/30 hover:bg-neon-cyan/10 text-xs py-1 px-3"
            ...
            {form.experience.map((item, index) => (
              <div key={`experience-${index}`} className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-background/50 p-4 rounded-xl border border-white/5">
                {['position', 'company'].map((key) => (
                  <input
                    key={key}
                    className="bg-surface-container border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-neon-purple focus:shadow-glow-purple transition-all"
                ...
                />
                <textarea
                  className="md:col-span-2 bg-surface-container border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-neon-purple focus:shadow-glow-purple transition-all resize-none"
                  placeholder="Highlights and responsibilities"
                  value={item.description || ''}
                  onChange={(e) => updateListItem('experience', index, 'description', e.target.value)}
                  rows={2}
                />
                <button type="button" className="text-neon-pink text-left text-xs font-bold hover:underline w-max" onClick={() => removeListItem('experience', index)}>
                  Remove experience
                </button>
              </div>
            ))}
          ...
          <fieldset className="border border-white/10 rounded-2xl p-6 space-y-6 relative z-10 bg-surface-container/50">
            <div className="flex items-center justify-between">
              <legend className="text-[10px] font-bold text-neon-cyan uppercase tracking-wider">Education</legend>
              <Button
                type="button"
                variant="ghost"
                className="text-neon-cyan border border-neon-cyan/30 hover:bg-neon-cyan/10 text-xs py-1 px-3"
            ...
            {form.education.map((item, index) => (
              <div key={`education-${index}`} className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-background/50 p-4 rounded-xl border border-white/5">
                ...
                <button type="button" className="text-neon-pink text-left text-xs font-bold hover:underline w-max" onClick={() => removeListItem('education', index)}>
                  Remove education
                </button>
              </div>
            ))}
          ...
          <div className="flex gap-4 pt-2 relative z-10">
            <Button type="submit" variant="primary" disabled={isSaving} className="bg-gradient-to-r from-neon-purple to-neon-pink border-none text-white shadow-glow-purple">
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-8">
          {/* Experience Card */}
          <section className="glass-card rounded-2xl p-8 hover:border-neon-purple/50 transition-all">
                  ...
                  <div key={i} className="flex gap-6 group">
                    ...
                    <div className={`flex-1 ${i < experience.length - 1 ? 'pb-8' : ''}`}>
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h4 className="font-bold text-lg text-white group-hover:text-neon-cyan transition-colors">{exp.position}</h4>
                          <p className="font-bold text-sm text-on-surface-variant">{exp.company}</p>
                        </div>
                        <span className="text-[10px] font-bold uppercase tracking-wider px-3 py-1 bg-surface-container-high rounded-full border border-white/5 text-neon-mint">
          ...
          <section className="glass-card rounded-2xl p-8 hover:border-neon-purple/50 transition-all">
            ...
            {skills.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {skills.map(skill => (
                  <span key={skill} className="text-[10px] font-bold bg-neon-purple/10 text-neon-purple px-4 py-1.5 rounded-full border border-neon-purple/30 uppercase tracking-wider shadow-glow-purple">
                    {skill}
                  </span>
                ))}
              </div>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-8">
          {/* Bio */}
          <section className="glass-card rounded-2xl p-6 hover:border-neon-purple/50 transition-all">
          ...
          <section className="glass-card rounded-2xl p-6 hover:border-neon-purple/50 transition-all">
            ...
            {profile?.resumeUrl ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="bg-surface-container border border-white/10 hover:border-neon-cyan/50 hover:bg-neon-cyan/5 rounded-xl p-4 flex items-center gap-4 transition-all group"
              >
                <FileText size={24} className="text-neon-cyan group-hover:shadow-glow-cyan" />
                <p className="font-bold text-sm text-white truncate group-hover:text-neon-cyan transition-colors">View current CV</p>
              </a>
            ) : (
          ...
          <section className="glass-card rounded-2xl p-6 hover:border-neon-purple/50 transition-all">
            ...
            <ul className="space-y-6">
              <li className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-surface-container border border-white/10 flex items-center justify-center text-neon-orange shadow-inner">
                  <Mail size={16} />
                </div>
                <div>
              ...
              <li className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-surface-container border border-white/10 flex items-center justify-center text-neon-mint shadow-inner">
                  <Phone size={16} />
                </div>
                <div>
              ...
              <div className="space-y-3">
                {Object.entries(profile.socialLinks).filter(([, value]) => Boolean(value)).map(([label, value]) => (
                  <a
                    ...
                    className="flex items-center justify-between text-neon-cyan hover:underline capitalize font-bold text-sm p-3 bg-surface-container-high/30 rounded-xl border border-white/5 hover:border-neon-cyan/30 transition-all"
                  >
                    <span>{label}</span>
      ...
      <div className="mt-8 space-y-8">
        ...
        <section className="bg-neon-pink/5 rounded-2xl p-6 border border-neon-pink/20 space-y-4">
          <h3 className="font-bold text-xl text-neon-pink">Danger Zone</h3>
          <Button variant="ghost" className="border border-neon-pink/50 text-neon-pink hover:bg-neon-pink hover:text-black transition-all" onClick={handleDeleteProfile}>
            Delete Profile Data
          </Button>
        </section>
      </div>
