...
export function ChatInbox() {
  ...
  return (
    <div className="flex -mt-lg -mx-[24px] lg:-mx-[32px] h-[calc(100vh-80px)] bg-background relative overflow-hidden">
      ...
      {/* Inbox Panel (Left ~30%) */}
      <section className={`w-full md:w-[320px] lg:w-[30%] h-full glass-card-pro border-white/5 md:border-r border-white/10 flex-col shrink-0 relative z-10 ${activeConversation ? 'hidden md:flex' : 'flex'}`}>
      ...
      <section className={`flex-1 h-full flex-col relative z-10 min-w-0 ${!activeConversation ? 'hidden md:flex' : 'flex'}`}>
        {!activeConversation ? (
          ...
          <>
            {/* Thread Header */}
            <header className="glass-card-pro border-white/5 border-b border-white/10 h-20 px-4 md:px-8 flex items-center justify-between shrink-0 z-10">
              <div className="flex items-center gap-3 md:gap-4">
                ...
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 text-white shadow-none flex items-center justify-center font-bold text-lg md:text-xl border border-white/20">
                  {getOtherParticipant(activeConversation).name?.[0]?.toUpperCase() || '?'}
                </div>
                <div>
                  <h3 className="font-bold text-base md:text-lg text-white line-clamp-1">{getOtherParticipant(activeConversation).name}</h3>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant mt-0.5 line-clamp-1">
