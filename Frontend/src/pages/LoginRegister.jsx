...
export function LoginRegister() {
    ...
    try {
      ...
      if (resultAction.error) {
        if (Array.isArray(resultAction.payload)) {
          resultAction.payload.forEach(err => toast.error(err));
        } else {
  ...
  return (
    <main className="min-h-screen w-full bg-[#0A0F1A] text-slate-200 relative overflow-hidden flex flex-col md:flex-row">
      ...
      {/* Top Navigation */}
      <nav className="absolute top-0 left-0 w-full p-4 md:p-8 flex justify-between items-center z-50 pointer-events-none">
        <div className="absolute top-6 left-6 md:top-12 md:left-12 z-20 flex items-center">
          <img src="/Logo.svg" alt="HireHub Logo" className="h-10 md:h-16 w-auto -ml-2 md:-ml-4 -mr-4 md:-mr-12 scale-125" />
          <span className="font-bold text-2xl md:text-3xl text-white tracking-tight drop-shadow-md ml-2 md:ml-0">HireHub</span>
        </div>
      </nav>

      {/* Left Section: Form Area */}
      <section className="w-full md:w-1/2 flex items-center justify-center p-4 sm:p-8 md:p-12 lg:p-24 relative z-20">
        <div className="w-full max-w-lg mt-20 md:mt-0 animate-slide-up" style={{ animationDelay: '0.1s' }}>
                ...
                <div className="mb-6">
                  <div className="grid grid-cols-2 gap-4">
                    <label className="cursor-pointer group/role">
                      <input {...register('role')} type="radio" value="candidate" className="sr-only peer" />
                      <div className={`p-4 rounded-2xl border ${errors.role ? 'border-red-500/50' : 'border-white/5'} bg-black/20 peer-checked:border-emerald-500/50 peer-checked:bg-emerald-500/10 transition-all text-center group-hover/role:border-white/10`}>
                    ...
                    <label className="cursor-pointer group/role">
                      <input {...register('role')} type="radio" value="company" className="sr-only peer" />
                      <div className={`p-4 rounded-2xl border ${errors.role ? 'border-red-500/50' : 'border-white/5'} bg-black/20 peer-checked:border-amber-500/50 peer-checked:bg-amber-500/10 transition-all text-center group-hover/role:border-white/10`}>
                  ...
                  {errors.role && <p className="text-red-400 text-xs mt-2 ml-1">{errors.role.message}</p>}
                </div>
              )}
